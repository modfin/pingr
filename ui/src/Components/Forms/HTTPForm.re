...
let rules = [
  ...
  (ReqHeaders, [(Form.Custom(keyPairValidation), keyMsg)]),
  (ResHeaders, [(Form.Custom(keyPairValidation), keyMsg)]),
];
...
let getPayload = (~inputTest=?, values) => {
  ...
  FormHelpers.setJsonKey(payload, "test_type", Str("HTTP"));
    ...
    initialState={
                   ...
                   | Some(httpTest) =>
                     ...
                     | HTTP(http) =>
                       ...
                       | Some(headers) =>
                         init.reqHeaders =
                           List(Js.Dict.entries(headers) |> Array.to_list)
                       };
                       switch (http.reqBody) {
                       | None => ()
